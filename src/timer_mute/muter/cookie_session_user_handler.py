...
class CookieSessionUserHandler(AuthBase):
    ...
    def __init__(
        ...
        Raises:
            ...
            ValueError: 認証フローの途中でエラーが発生し、ログインに失敗した
        """
        ...
        if (
            ...
            raise ValueError("Failed to get auth_token or ct0 from Cookie")
    ...
    def logout(self) -> None:
        ...
        try:
            ...
            raise ValueError("Failed to logout (failed to parse response)")
        if status != "ok":
            raise ValueError(f"Failed to logout (status: {status})")
    ...
    def _get_tweepy_exception(self, response: requests.Response):
        """
    ...
    def _get_guest_token(self) -> str:
        ...
        try:
            ...
            raise ValueError("Failed to get guest token")
    ...
    def _login(self) -> None:
                ...
                if response.status_code < 400:
                    for subtask in data["subtasks"]:
                        if subtask["subtask_id"] == subtask_id:
                            return subtask
                    raise ValueError(f"{subtask_id} not found in response")
            raise self._get_tweepy_exception(response)
        ...
        if js_inst_response.status_code != 200:
            raise ValueError("Failed to get js_inst")
        ...
        if flow_04_response.json()["status"] != "success":
            raise ValueError(f"Failed to login (status: {flow_04_response.json()['status']})")
