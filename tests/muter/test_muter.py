...
class TestMuter(unittest.TestCase):
    ...
    def test_init(self):
        mock_account = self.enterContext(patch("timermute.muter.muter.Account"))
        ...
        self.assertEqual((Path("./timermute/muter/muter").parent / "cache/").resolve(), instance.cache_path)
    ...
    def test_get_mute_keyword_list(self):
        self.enterContext(patch("timermute.muter.muter.logger.info"))
        mock_account = self.enterContext(patch("timermute.muter.muter.Account"))
        mock_config = self._get_mock_config()
        self._reset_muter()
        instance = Muter(mock_config)
    ...
    def test_unmute_keyword(self):
        self.enterContext(patch("timermute.muter.muter.logger.info"))
        mock_account = self.enterContext(patch("timermute.muter.muter.Account"))
        mock_get_mute_keyword_list = self.enterContext(patch("timermute.muter.muter.Muter.get_mute_keyword_list"))
